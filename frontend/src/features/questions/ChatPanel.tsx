...
function Turn({ turn, onOpenSource, onOpenGeneratedDocument, onAskConcept, onRetry, onEdit }: { turn: QuestionResult; onOpenSource: (source: QuestionSource) => void; onOpenGeneratedDocument: (id: number) => void; onAskConcept: (value: string) => void; onRetry: (id: number) => void; onEdit: (question: string) => void }) {
      ...
      <div className="chat-assistant-message">
        ...
        {!pending && !noEvidence && !failed && (
          <div className="answer-block chat-answer-block">
            ...
            <div className="answer-copy">{renderAnswer(turn.answer_markdown ?? '')}</div>
...
export function ChatPanel({ conversation, latestResult, onBackToHistory, onOpenSource, onOpenGeneratedDocument, onAskConcept, onAsk, loading, onRetry, onEdit, onNewConversation, onRename, onDelete }: ChatPanelProps) {
  const [draft, setDraft] = useState('')
  const turns = conversation?.turns.length ? conversation.turns : [latestResult]
  ...
  return (
    <aside className="context-panel question-panel chat-panel" role="dialog" aria-modal="true" aria-label="AI 대화">
      ...
      <div className="chat-thread" aria-live="polite">
        {turns.map((turn) => <Turn key={turn.id} turn={turn} onOpenSource={onOpenSource} onOpenGeneratedDocument={onOpenGeneratedDocument} onAskConcept={onAskConcept} onRetry={onRetry} onEdit={onEdit} />)}
      </div>
