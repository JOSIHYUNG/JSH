...
## 13. 탐색형 Agent 아키텍처 추가

### 13.1 모듈 경계

- agent/orchestrator.py: 최대 30 cycle state machine과 terminal 처리
- agent/registry.py: strict schema 기반 custom tool registry
- agent/tools/search_knowledge.py: 기존 RetrievalService adapter
- agent/tools/explore_node.py: graph read와 alias mention range merge
- agent/tools/web_search.py: hosted tool output·citation parser
- agent/prompt_loader.py: 외부 markdown/json prompt 및 version 검증
- agent/events.py와 repository: durable activity event와 SSE replay
- integrations/openai/agent_responses.py: Responses API 경계

QuestionService는 conversation/turn/history facade로 축소하고 질문 실행은 Orchestrator가 소유한다. route에서 SDK를 직접 호출하지 않는다.

### 13.2 실행 정책

현재 질문과 최근 완료 turn 최대 3개만 context로 조립하며 summary를 만들지 않는다. parallel_tool_calls=false로 한 cycle에 tool 하나만 실행한다. tool 오류는 stable error envelope로 모델에 재주입하고, 30번째 tool 요청 뒤에는 추가 LLM 호출 없이 max_turns로 종료한다.

### 13.3 복구·보안

Agent run claim은 transaction으로 단일 실행을 보장한다. startup에서 stale running run을 SERVICE_RESTARTED로 종료하고, event에는 safe label·bounded JSON만 저장한다. 문서와 web 결과는 untrusted data로 주입하며 prompt injection을 지시문으로 실행하지 않는다.
